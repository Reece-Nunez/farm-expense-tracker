...
import { createIncome as createIncomeMutation } from "@/graphql/mutations";
...
export default function ImportIncomeCSV() {
  ...
  const handleReviewSubmit = async (finalRows) => {
    try {
      ...
      toast.success(`Successfully imported ${incomesToSave.length} income records!`);
      navigate("/dashboard/income");
    } catch (err) {
      console.error("Error saving Income to AppSync:", err);
      ...
      <>
        <div>
          ...
          <input
            ...
            className="block w-full text-sm text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-green-500 file:text-white hover:file:bg-green-600 transition-colors cursor-pointer"
          />
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-100 to-green-300 flex items-center justify-center p-4">
      <Card className="w-full max-w-6xl shadow-2xl rounded-3xl overflow-hidden">
        <CardHeader className="flex flex-col items-center justify-center bg-white p-6 shadow-sm">
          ...
          <h1 className="text-3xl font-extrabold text-green-700 mb-3">Import Income via CSV</h1>
